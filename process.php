...
if ($result=="") {
  try {
    $stmt = $pdo->prepare("INSERT INTO `orders` (`name`, `address`, `email`, `number`, `qty1`, `qty2`, `qty3`, `qty4`, `qty5`, `message` ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
    $stmt->execute([$_POST["name"], $_POST["address"], $_POST["email"], $_POST["number"], $_POST["qty1"], $_POST["qty2"], $_POST["qty3"], $_POST["qty4"], $_POST["qty5"], $_POST["message"]]);
  } catch (Exception $ex) { $result = $ex->getMessage(); }
}
